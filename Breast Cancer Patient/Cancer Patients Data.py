...
#Lgoin
web = RoboBrowser(parser='html.parser')
web.open("http://visual.ic.uff.br/dmi/")
form = web.get_form()
#user_id 
form['usuario'] = "web1621"
#submit form 
web.submit_form(form)
src = str(web.parsed())
...
while page <= max_links:
    ...
    web.open("http://visual.ic.uff.br/dmi/prontuario/details.php?id=" +str(page))
    src1 = str(web.parsed())
    
    #loop to get user id and user age
    #goto this tag "div" having 'class' "description"
    
    for implink in web.find_all('div', {'class': 'descripcion1'}):
    ...
    for diag in web.find_all('div', {'class': 'visitauser'}):
    ...
    i = 0
    for img in web.find_all('div', {'class': 'imagem'}):
